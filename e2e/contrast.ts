 ...
 *    are `color-mix(in oklab, tone …%, var(--surface))`, and so are all three
 *    `.pill` states, the `.callout-caveat` and `.callout-danger` scoping
 *    warnings, the `.cl-hero-why` accent wash, and the `data-mark` fills on
 *    `.ct-tile` that say whether a guess hit, missed, or whether a record has
 *    been folded into the answer. Those fills hold every retrieval verdict,
 *    every failure code, the collusion alarm and the whole
 *    what-is-and-is-not-hidden table. The shared top bar adds a
 *    `color-mix(in srgb, ...)` for its ink. A violations-only gate therefore
 *    measured the contrast of almost none of this lab's conclusions.
 *
 *  - TEXT FADED BY AN ANCESTOR'S `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. This stylesheet declares
 *    NO `opacity` anywhere on purpose: every muted tone is a dimmer colour
 *    token (`--text-dim`), because opacity multiplies through ancestors and a
 *    value that measures AA in isolation can fall below it inside a tinted
 *    card. `.btn:disabled` recolours rather than fading, for the same reason.
 *    The walk carries the opacity model regardless, so the first `opacity`
 *    anyone adds is measured on arrival rather than on the next audit.
 *
 ...
 *    real scrollers and they are LOAD-BEARING: `.table-wrap { overflow-x:
 *    auto }` wraps the head-to-head comparison, the byte-to-coefficient packing
 *    table, the predicted-budget table, the observer log and the
 *    what-is-hidden table, every one of which is wider than a phone. Content
 ...
 *    nothing behind it, fall through to WHITE, and invent a failure against a
 *    page whose canvas is near-black.
 *
 ...
 * `::marker`, the UA's own, recoloured but not replaced, and the custom
 * `<select>` chevron is a pair of background gradients rather than a
 * pseudo-element), so that half of `nontext.ts` is inert here — and it runs at
 ...
 * below. This page hides the verdict glyphs (✓ ✕ ⚠ ·) that sit beside their
 * own words and the `#n` index labels on the ciphertext and record tiles, all
 * of which are painted in semantic inks on `color-mix()` tints, so `gate.ts`
 * measures every `aria-hidden` subtree explicitly with that exemption lifted
 * rather than reasoning about which glyphs matter.
 */
...
export async function auditContrast(
  ...
  return page.evaluate(([rootSelector, allowAriaHidden]: [string, boolean]) => {
     ...
     * the tile grids: the shelf renders 64 `.shelf-item` buttons with three
     * text spans each, and both the Server's View and Homomorphic Selection
     * panels render a further 64 `.ct-tile`s apiece — nearly 400 sibling
     * elements re-walking the same ancestors up to `<body>`, plus five wide
     * `.table-wrap` tables. Without the caches the pass re-reads the same
     ...
     * that is the `.table-wrap { overflow-x: auto }` regions, which are live
     * and load-bearing: the head-to-head comparison, the packing table, the
     * predicted-budget table, the observer log and the what-is-hidden table all
     * scroll sideways on a narrow viewport. The collection is built from the
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // of the shape: five `details.disclose` panels ship SHUT — the
      // byte-to-coefficient packing table, the fresh-randomness argument, the
      // three-line derivation of why the sum selects, the two-server cost
      // discussion and the malformed-query note. The gate opens each by
      ...
      // every `role="status"` live region here — the fold progress line, the
      // guess verdict, the trial result, the head-to-head table, the retrieval
      // verdict, the failure-code readout and the observer log — is a VISIBLE
     ...
     *
     * Every `aria-hidden` element on this page is decorative and enumerated:
     * the `.verdict-icon` glyph (✓ ✕ ⚠ ·) on each verdict, the leading glyph
     * inside each `.pill`, and the `.ct-tile-idx` "#n" label on every
     * ciphertext and record tile — whose position is already in the tile's own
     * `aria-label` — plus the shared header's two SVG marks, which carry no
     * text. Each duplicates words or a label directly beside it, but each is
     * painted in a SEMANTIC ink (`--ok-text`, `--bad-text`, `--alarm-text`) or
     * on a `color-mix()` tint, which is why `scan()` runs the whole
     * `aria-hidden` set through this walk with the exemption lifted rather than
     * arguing any of them is merely decorative.
     *
     * Nothing on this page hides a VALUE. No budget figure, byte count, failure
     * code, retrieved record or comparison cell is inside an `aria-hidden`
