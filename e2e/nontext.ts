 ...
 * as their own accent fill, so they have no edge of their own and live or die
 * by fill-vs-surround; every `:hover` state repaints the fill as a
 * `color-mix()`; and the `.ct-tile[data-mark]` and `.pill-*` states recolour
 * both border and fill toward a semantic tone. This oracle judges each control
 * as painted, at every driven state — including the folded-tile, guess-hit and
 * guess-miss recolourings only the drive reaches.
 *
 * The generated-content half is inert in this repo today — the stylesheet
 * declares no `content` at all; the disclosure triangles on `<summary>` are
 * `::marker`, the UA's own, recoloured but not replaced, and the custom
 * `<select>` chevron is drawn with background gradients rather than a
...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * declares gradients in exactly one place — the two 45-degree wedges that
     * draw the custom `<select>` chevron after `appearance: none` removes the
     * native one — and everything else is flat hex or `color-mix()`, which
     * `resolve` handles as a flat colour. So this sampler is live on the three
     * selects and dormant elsewhere, which is the right way round: the chevron
     * is precisely the control whose fill a naive reader would get wrong.
     */
     ...
     * tile grids: 64 `.shelf-item` buttons on the shelf and a further 64
     * `.ct-tile` controls on each of two other panels, every one of them a
     * bordered control this oracle has to judge — all siblings re-walking the
     * same ancestors up to `<body>`. Without the caches the pass re-reads the
     * same computed styles and rects tens of thousands of times. Nothing
     * mutates the DOM during the pass, so the cached values cannot go stale.
     */
    const styleCache = new Map<Element, CSSStyleDeclaration>();

    // Declared by the paint core shared with `contrast.ts`; referenced here so
    // the shared block compiles unchanged when only half of it is used.
    void splitTopLevel;
    void over;
    void fade;
    void luminance;
    ...
    for (const el of Array.from(document.querySelectorAll(CONTROL))) {
      ...
      // This lab's tabs and tiles happen to paint all four sides today; the
      // per-side walk is what keeps that an implementation detail rather than a
      // load-bearing assumption. It matters most for `.verdict`, which declares
      // `border: 1px solid transparent` and then recolours it per tone — four
      // painted sides in every state the drive reaches, none in the base rule.
      //
      ...
      // 1px a coloured state will later occupy so nothing shifts. Counting
      // such a spacer as a border would make an element with no fill and no
      // painted edge — identified by its text alone, exactly the case the "is
      // it trying to draw itself as a control?" test below exists to exclude —
      // report 1.00:1.
      ...
      for (const side of paintedSides) {
        ...
        // fill as its backdrop. This page has exactly that shape: one
        // `--control-border` token edges the neutral `.btn`, the accent-filled
        // `.btn-primary`, the tinted `.pill-ok` / `.pill-bad` / `.pill-warn`
        // and the semantic `.ct-tile[data-mark]` states, and each of those
        // fills is what the edge is actually painted over.
